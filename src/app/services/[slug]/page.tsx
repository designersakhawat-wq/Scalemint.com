...
// Fallback Rich Service Database
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const features = service.features || [
    { title: "Strategic Planning", desc: "Tailored approach designed for maximum ROI." },
    { title: "Execution & Management", desc: "End-to-end delivery handled by our expert team." },
    { title: "Performance Optimization", desc: "Continuous iteration to scale results." },
  ];

  const processSteps = service.process || [
    { step: "STEP 01", title: "Audit & Strategy", desc: "Analyze business goals and opportunities.", color: "bg-yellow-500/10 text-yellow-400" },
    { step: "STEP 02", title: "Design & Setup", desc: "Build tailored creative and strategic assets.", color: "bg-orange-500/10 text-orange-400" },
    { step: "STEP 03", title: "Execution & Launch", desc: "Deliver campaigns with precision tracking.", color: "bg-green-500/10 text-green-400" },
    { step: "STEP 04", title: "Scale & Grow", desc: "Optimize metrics to expand your bottom line.", color: "bg-rose-500/10 text-rose-400" },
  ];

  const faqs = service.faqs || [
    { title: "Customized Strategy", content: "Every brand receives a tailored approach built to achieve measurable business growth." },
    { title: "Dedicated Support", content: "You get direct communication with our senior specialists throughout the collaboration." },
  ];

  return (
    <main className="min-h-screen bg-[#040822] flex flex-col">
      {/* Hero Section */}
      <section className="bg-[#040822] relative overflow-hidden pt-32 pb-32">
        <Navbar theme="dark" />
        ...
        <div className="absolute right-0 top-1/4 -z-10 h-[500px] w-[500px] rounded-full bg-brand-electric opacity-20 blur-[100px]"></div>
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mt-12">
          ...
          <div data-aos="fade-right" className="flex flex-col items-start">
            ...
            <div className="flex items-center gap-3 text-white/50 text-sm font-medium mb-8 tracking-wide uppercase">
              ...
              <span>/</span>
              <span className="text-brand-electric">{service.title}</span>
            </div>

            <div className="max-w-xl">
              <h1 className="text-5xl md:text-6xl font-bold text-white leading-[1.15] tracking-tight mb-8">
                {service.heroHeadline || service.title}
              </h1>
              <p className="text-lg text-slate-300 leading-relaxed mb-10 border-l-2 border-brand-electric pl-5">
                {service.description}
              </p>
              
              <Link href="/contact-us" className="inline-flex items-center gap-3 bg-brand-electric text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-white hover:text-[#040822] transition-all duration-300 shadow-[0_0_40px_rgba(48,255,151,0.3)] group">
                Book a Call
      ...
      <div className="bg-[#040822] relative z-20 pb-16">
        <ProjectCarousel />
      </div>
      
      {/* Content Section */}
      <section className="bg-[#040822] relative z-30 pt-16 pb-24 px-8 overflow-hidden">
        
        {/* Trusted Brands Marquee */}
        <div className="max-w-[100vw] mx-auto mb-24 text-center border-b border-white/10 pb-20">
          ...
          <div data-aos="fade-up" className="lg:col-span-5 flex flex-col items-start">
            <div className="sticky top-32">
              ...
              <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-6">
                {service.whyTitle || `Why choose us for ${service.title}?`}
              </h2>
              <p className="text-slate-300 text-lg leading-relaxed mb-8">
                {service.whyText || service.description}
              </p>
              
              <div className="bg-[#0a0f2c] border border-white/10 rounded-3xl p-8 mt-8">
                <h3 className="font-bold text-xl text-white mb-4">Ready to scale?</h3>
                <p className="text-slate-400 mb-6">Our team is ready to analyze your current setup and provide a custom strategy.</p>
                <Link href="/contact-us" className="inline-flex items-center gap-2 text-brand-electric font-bold hover:gap-4 transition-all">
                  Let&apos;s Talk <span className="text-xl">&rarr;</span>
                </Link>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-8">
              {features.map((feature: any, idx: number) => (
                <div key={idx} data-aos="fade-left" data-aos-delay={idx * 150} className="flex gap-6 group">
                  ...
                  <div>
                    <h4 className="text-2xl font-bold text-white mb-3 group-hover:text-brand-electric transition-colors">{feature.title}</h4>
      ...
      <section className="bg-[#040822] py-24 relative z-40 border-t border-white/5">
        <div className="max-w-[1400px] mx-auto px-8">
          
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-6">
                Let&apos;s Walk You Through Our <br/><span className="text-brand-electric">{service.title}</span> Process
              </h2>
              <p className="text-slate-400 text-lg">
                The process of {(service.title || "").toLowerCase()} follows a certain chronological order to ensure maximum results.
              </p>
            </div>
            <Link href="/contact-us" className="shrink-0 bg-white text-[#040822] px-8 py-4 rounded-full font-bold hover:bg-brand-electric hover:text-white transition-all shadow-lg">
              Schedule a Call Now
            </Link>
          </div>
          ...
          <div className="relative w-full overflow-hidden mask-image-gradient">
            ...
            <AutoScrollCarousel speed={1}>
              {processSteps.map((step: any, idx: number) => (
                <div key={idx} className="shrink-0 w-[340px] md:w-[400px] bg-[#0a0f2c] border border-white/5 rounded-3xl p-10 hover:border-brand-electric/50 transition-colors group">
                  <div className={`inline-block px-5 py-2 rounded-full text-xs font-bold mb-8 ${step.color || "bg-brand-electric/10 text-brand-electric"}`}>
                    {step.step || `STEP 0${idx + 1}`}
                  </div>
                  <h4 className="text-2xl font-bold text-white mb-5 group-hover:text-brand-electric transition-colors pr-4">
                    {step.title}
                  </h4>
      ...
      <section className="bg-[#040822] py-24 relative z-40 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-8">
          ...
          <div className="mb-16">
            <div className="inline-block border border-brand-electric text-brand-electric rounded-full px-5 py-2 text-sm font-medium mb-8">
              Why Scaleminte for {service.title}
            </div>
            <h2 className="text-4xl md:text-5xl lg:text-6xl text-white leading-[1.2] max-w-4xl font-bold tracking-tight">
              <span className="font-serif italic font-medium tracking-normal pr-2">Smart Design</span> Meets Speed, Strategy and <span className="font-serif italic font-medium tracking-normal pl-1">Real Results</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            {/* Left: Image */}
            <div className="rounded-[2.5rem] overflow-hidden relative h-[400px] lg:h-[500px] w-full group">
              <img 
                src={service.benefitsImage || service.img || "/images/startup.jpg"} 
                alt={`${service.title} Benefits`}
                className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"
              />
              <div className="absolute inset-0 bg-brand-electric/10 group-hover:bg-transparent transition-colors duration-500"></div>
            </div>
