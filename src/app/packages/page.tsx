            ...
            {packages.map((pkg, idx) => (
              ...
              >
                ...
                <div className="mb-8">
                  ...
                  {pkg.price !== "Custom" && <span className="text-slate-400 text-sm ml-2">/ month</span>}
                </div>
                ...
                <Button 
                  href={pkg.buttonLink || `/contact-us?plan=${encodeURIComponent(pkg.name)}`} 
                  variant={pkg.isPopular ? 'primary' : 'outline'} 
                  className="w-full py-4 text-center justify-center font-bold"
                >
                  {pkg.buttonText || `Get Started with ${pkg.name}`}
                </Button>
              </div>
            ))}
          </div>
