...
import { useSiteConfig, sanitizeImageUrl } from "@/context/SiteConfigContext";
...
export default function ProjectCarousel() {
  ...
  const rawCarousel = settings.images?.heroCarousel;
  const carouselProjects = Array.isArray(rawCarousel) && rawCarousel.length > 0
    ? rawCarousel
    : [
        { id: "c1", title: "Educational Design", url: DEFAULT_PROJECT_IMAGES[0], position: "center" },
        { id: "c2", title: "SaaS Dashboard", url: DEFAULT_PROJECT_IMAGES[1], position: "center" },
        { id: "c3", title: "E-Commerce App", url: DEFAULT_PROJECT_IMAGES[2], position: "center" },
        { id: "c4", title: "Corporate Identity", url: DEFAULT_PROJECT_IMAGES[3], position: "center" },
        { id: "c5", title: "Social Media Platform", url: DEFAULT_PROJECT_IMAGES[4], position: "center" },
      ];
    ...
    <div data-aos="fade-up" data-aos-delay="600" className="relative -mt-24 z-20 w-full overflow-hidden pb-16 group mask-image-gradient">
      ...
      <AutoScrollCarousel speed={1}>
        {carouselProjects.map((project, idx) => {
          const fallback = DEFAULT_PROJECT_IMAGES[idx % DEFAULT_PROJECT_IMAGES.length];
          const imgUrl = sanitizeImageUrl(project.url, fallback);
