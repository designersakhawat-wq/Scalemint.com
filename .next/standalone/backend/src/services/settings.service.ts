...
export interface SiteSettings {
  ...
  // Dedicated Navbar Mega Menu Customizer
  navbarMenu: {
    servicesDropdown: {
      navLabel: string;
      sidebarLabel: string;
  ...
  // Dedicated Footer Studio Configuration
  footerConfig: {
    tagline: string;
    description: string;
    copyright: string;
  ...
  // Dedicated Legal Pages (Terms & Privacy)
  legalPages: {
...
function getBaseSettings(): SiteSettings {
  const raw = loadData<Partial<SiteSettings>>(SETTINGS_FILE, defaultSettings);
  return {
    ...defaultSettings,
    ...(raw || {}),
    navbarMenu: {
...
export class SettingsService {
  ...
  async updateSettings(newSettings: Partial<SiteSettings>): Promise<SiteSettings> {
    ...
    saveData<SiteSettings>(SETTINGS_FILE, merged);
    return merged;
  }
}
